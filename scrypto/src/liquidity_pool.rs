    ...
    impl LiquidityPool {
        pub fn instantiate_liquidity_pool(
            ...
            let (address_reservation, component_address) = Runtime::allocate_component_address(
                LiquidityPool::blueprint_id()
            );

            // Sorting the buckets and then creating the hashmap of the vaults from the sorted buckets
            let (bucket_a, bucket_b): (FungibleBucket, FungibleBucket) = sort_buckets(
                bucket_a,
                bucket_b
            );
            let addresses: (ResourceAddress, ResourceAddress) = (
                bucket_a.resource_address(),
                bucket_b.resource_address(),
            );
        ...
        pub fn other_resource_address(&self, resource_address: ResourceAddress) -> ResourceAddress {
        ...
        pub fn swap(&mut self, input_tokens: FungibleBucket) -> FungibleBucket {
            let output_resource_address = self.other_resource_address(
                input_tokens.resource_address()
            );
            ...
            // and the pool fees
            let output_amount: Decimal =
                (output_tokens_vault_amount * (dec!("1") - self.fee) * input_tokens.amount()) /
                (input_tokens_vault_amount + input_tokens.amount() * (dec!("1") - self.fee));
        ...
        pub fn add_liquidity(
            ...
            let (mut bucket_a, mut bucket_b): (FungibleBucket, FungibleBucket) = sort_buckets(
                token1,
                token2
            );
            ...
            //the vaults
            let (amount_a, amount_b): (Decimal, Decimal) = if
                (m == Decimal::zero()) |
                (n == Decimal::zero()) |
                (m / n == dm / dn)
            {
            ...
            let pool_units_amount: Decimal = if
                self.pool_units_resource_manager.total_supply().unwrap() == Decimal::zero()
            {
                dec!("100.00")
            } else {
                (amount_a * self.pool_units_resource_manager.total_supply().unwrap()) / m
            };
            let pool_units: FungibleBucket = self.pool_units_resource_manager
